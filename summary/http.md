...
​		HTTPS是一种通过计算机网络进行安全通信的传输协议，经由HTTP进行通信，利用SSL/TLS建立全信道，加密数据包。HTTPS使用的主要目的是提供对网站服务器的**身份认证安全性**，同时保护交换数据的**隐私与完整性**。
...
| 端口     | 80                                                           | 443                                                          |
| 适用场景 |                                                              | 登陆、支付                                                   |
| 缺点     | 明文传输容易被窃听截取；数据的完整性未校验易被篡改；没有验证对方身份，存在冒充危险 | HTTPS协议多次握手导致页面的加载时间延长；连接缓存不如HTTP高效，会增加数据开销和功耗； 申请SSL证书需要钱； SSL涉及到的安全算法会消耗 CPU 资源，对服务器资源消耗较大。 |
| 优点     |                                                              |                                                              |
